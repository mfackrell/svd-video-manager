...
def svd_video_manager(request):
    ...
    if data.get("status") == "COMPLETED":
        ...
        if job["loop"] >= TOTAL_LOOPS:
            job["status"] = "COMPLETE"
            job["final_video_url"] = stitch_chunks_to_final(bucket, root_id, job["chunks"])
            job_blob.upload_from_string(json.dumps(job))
            return {"status": "finished", "final_video_url": job["final_video_url"]}, 200
    ...
    # INITIAL TRIGGER (from orchestrator)
